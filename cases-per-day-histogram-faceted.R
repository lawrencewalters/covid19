...
number_of_days <- 40
regression_days <- 20
countries <- c('DE','US', 'CA','DE-BER', 'US-SKO')
...
skokie <- CSVData("data\\skokie.csv", cumulativeCounts = FALSE, countriesAndTerritories = "Skokie", geoId = "US-SKO", countryterritoryCode = "US", popData2019 = 60000, continentExp = "North America" )
ecdc <- rbind(skokie, ecdc)
...
ggplot(subset(ecdc, 
  ...
  ggtitle(paste("New cases per day for last", number_of_days,"days as of",format.Date(latest_data_date, "%D")),
          subtitle = paste("p value from linear regression on most recent",regression_days,"days. skokie.org data, ECDC data",
                           latest_data_date,
                           "retrieved",
